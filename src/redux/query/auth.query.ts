...
export const authApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    logoutFcm: builder.mutation<any, { token: string }>({
      query: (data) => ({
        url: "/api/services/app/PushNotification/LogoutFcm",
        method: "post",
        body: data,
      }),
      invalidatesTags: [{ type: "auth", id: "fcmtoken" }],
    }),
    registerToTenant: builder.mutation<any, { token: string; tenantId: number; appType: number }>({
      query: (data) => ({
        url: "/api/services/app/PushNotification/RegisterToTenant",
        method: "post",
        body: data,
      }),
      invalidatesTags: [{ type: "auth", id: "fcmtoken" }],
